...
class LCD1602
{
	private:
		...
		void Delay(uint32_t s);				//延时(循环实现)
		void Delay_Sec(uint8_t sec);			//延时(最小单位为一秒)
		void Write_Com(uint8_t com);			//写命令
		void Write_Data(uint8_t data);			//写数据
	public:
		void Init(int rs, int rw, int en,int d0, int d1, int d2, int d3,int d4, int d5, int d6, int d7);//初始化
		void Write_String(uint8_t x,uint8_t y,const char *s);	//写字符串
		void Shift(uint8_t dire, uint32_t speed, uint8_t len);	//字符位移 dire:移动方向 speed:移动速度(以秒为单位) len:移动长度
		void Clear(void);//清屏
}; 
